...
public:
    explicit RecordingReceiver(std::string name, bool blockUntilReleased = false)
        : name_(std::move(name)), blockUntilReleased_(blockUntilReleased) {}
    ...
    void PutTable(const LogTable& table) override {
        ...
        std::lock_guard lock(logMutex_);
        log_.push_back("PutTable:" + name_ + ":"
            + (marker != nullptr ? std::to_string(std::get<int64_t>(marker->value)) : "?"));
    }
    ...
    [[nodiscard]] bool HasEnteredPutTable() const {
        return enteredPutTable_.load(std::memory_order_relaxed);
    }
...
TEST(ReceiverThreadTest, AddDataReceiverAndCatchUpWhileRunningIsRaceFree) {
    ...
    EXPECT_EQ(early.Log(),
              (std::vector<std::string>{"Start:Early", "PutTable:Early:1", "PutTable:Early:2", "End:Early"}));
    ...
    EXPECT_EQ(late.Log(),
              (std::vector<std::string>{"Start:Late", "PutTable:Late:99", "PutTable:Late:2", "End:Late"}));
}
...
TEST(ReceiverThreadTest, AddDataReceiverAndCatchUpDeliversSynchronouslyAndIsNotDuplicated) {
    ...
    EXPECT_EQ(receiver.Log(),
              (std::vector<std::string>{"Start:R", "PutTable:R:42", "PutTable:R:7", "End:R"}));
}
