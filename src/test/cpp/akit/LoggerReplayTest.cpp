...
TEST_F(LoggerReplayParityTest, ReplayStartupLoadsInitialFrameAndWritesReplayOutputs) {
    StubReplaySource replaySource({
        ReplayFrame{
            1'000,
            [](LogTable& table) {
                table.GetSubtable("ReplayInputs").Put("Applied", 12.5);
            },
        },
    });
    ...
    EXPECT_DOUBLE_EQ(
        std::get<double>(Logger::GetCurrentStorage().values.at("ReplayInputs/Applied").value), 12.5);
    ...
    StubReplaySource replaySource({
        ReplayFrame{
            ...
            [](LogTable& table) {
                ...
                ds.Put("MatchType", frc::DriverStation::MatchType::kQualification);
                ds.Put("EventName", "Week Zero");
                ds.Put("MatchTime", 15.0);
                ds.GetSubtable("Joystick0").Put("Name", "Replay Pad");
                ds.GetSubtable("Joystick0").Put("Type", frc::GenericHID::HIDType::kHIDGamepad);
                ...
                std::vector<frc::Joystick::AxisType> axisTypes{frc::Joystick::AxisType::kYAxis};
                ds.GetSubtable("Joystick0").Put("AxisTypes", std::span<const frc::Joystick::AxisType>(axisTypes));
            },
        },
        ReplayFrame{
            2'000,
            [](LogTable& table) {
                ...
                ds.Put("MatchType", frc::DriverStation::MatchType::kElimination);
                ds.Put("EventName", "Replay Event");
                ds.Put("MatchTime", 9.5);
                ds.GetSubtable("Joystick0").Put("Name", "Updated Pad");
                ds.GetSubtable("Joystick0").Put("Type", frc::GenericHID::HIDType::kHIDJoystick);
                ...
                ds.GetSubtable("Joystick0").Put("AxisValues", std::span<const float>(axisValues));
                std::vector<frc::Joystick::AxisType> axisTypes{
                    frc::Joystick::AxisType::kZAxis,
                    frc::Joystick::AxisType::kTwistAxis,
                };
                ds.GetSubtable("Joystick0").Put("AxisTypes", std::span<const frc::Joystick::AxisType>(axisTypes));
            },
        },
...
TEST_F(LoggerReplayParityTest, ReplayExhaustionEndsLoggerExactlyOnce) {
    ...
    StubReplaySource replaySource({
        ReplayFrame{
            1'000,
            [](LogTable& table) {
                table.GetSubtable("DriverStation").Put("DSAttached", true);
            },
        },
    });
